...
const ThreadDetailPage: React.FC = () => {
  ...
  const handleDelete = () => {
    if (window.confirm('Delete this signal transmission permanently?')) {
    ...
    <Layout>
      <div className="space-y-6">
        ...
        <div className={`border rounded-3xl overflow-hidden shadow-2xl transition-all ${isDark ? 'bg-black border-rojo-900/30' : 'bg-white border-rojo-100'}`}>
          <div className="flex flex-col md:flex-row">
            <div className={`w-full md:w-48 p-8 flex flex-col items-center text-center border-b md:border-b-0 md:border-r ${isDark ? 'bg-rojo-950/20 border-rojo-900/20' : 'bg-rojo-50 border-rojo-100'}`}>
              ...
              <p className="font-black truncate w-full group-hover:text-rojo-500 transition-colors">@{thread.authorName}</p>
              <p className="text-[9px] font-black uppercase text-slate-500 mt-1">Originator</p>
            </div>
            <div className="flex-1 p-10 flex flex-col">
              ...
              <div className="flex items-center space-x-6 border-t border-rojo-900/10 pt-6">
                <button onClick={() => addReport(ReportType.THREAD, thread.id, 'Rules Violation', thread.content.substring(0, 50))} className="text-[10px] font-black uppercase text-slate-600 hover:text-rojo-500 flex items-center transition-colors">Flag Protocol Breach</button>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-4">
          <h2 className="text-[10px] font-black text-slate-600 uppercase tracking-widest">{threadPosts.length} Feedback Signals</h2>
                ...
                <div className="flex-1 p-6">
                  ...
                  <div className="flex items-center justify-between text-[10px] text-slate-600 font-bold uppercase">
                     ...
                     <div className="flex items-center space-x-6">
                        ...
                        <button onClick={() => addReport(ReportType.POST, post.id, 'Signal Violation', post.content.substring(0, 50))} className="hover:text-rojo-500 transition-colors">Flag</button>
                     </div>
                  </div>
                </div>
              </div>
        ...
        {!thread.isLocked ? (
          <div className={`border rounded-3xl p-10 shadow-2xl transition-all ${isDark ? 'bg-black border-rojo-900/30' : 'bg-white border-rojo-100'}`}>
            <h3 className="text-sm font-black uppercase text-slate-500 mb-6 flex items-center gap-2">Initiate Feedback Signal</h3>
            <textarea 
              value={replyText}
              ...
              placeholder="Join the synchronized stream..."
            />
          ...
          <div className={`border rounded-3xl p-12 text-center transition-all ${isDark ? 'bg-rojo-950/20 border-rojo-900/50 text-slate-500' : 'bg-rojo-50 border-rojo-100 text-slate-500'}`}>
            <p className="font-black uppercase text-xs tracking-[0.2em] text-rojo-500">Signal Channel Terminated</p>
            <p className="text-[10px] font-medium mt-2">Administrative override: This transmission is no longer accepting feedback.</p>
          </div>
        )}
      </div>
    </Layout>
