...
const ChatPage: React.FC = () => {
  ...
  // Only show users we have interacted with OR the user we just clicked from profile
  const filteredContacts = useMemo(() => {
    const list = users.filter(u => u.id !== currentUser?.id && (allChatPartners.includes(u.id) || u.id === targetIdFromUrl));
    return list;
      ...
      <div className={`border rounded-[2.5rem] overflow-hidden shadow-[0_0_50px_rgba(255,0,0,0.05)] h-[700px] flex transition-all ${isDark ? 'bg-black border-rojo-900/30' : 'bg-white border-rojo-100'}`}>
        <aside className={`w-80 flex flex-col border-r transition-all ${isDark ? 'border-rojo-900/20 bg-rojo-950/20' : 'border-slate-100 bg-slate-50/50'}`}>
          ...
          <div className="flex-1 overflow-y-auto no-scrollbar">
            {filteredContacts.map(user => (
              ...
              >
                <img src={user.avatarUrl} className={`w-12 h-12 rounded-2xl border-2 ${selectedUserId === user.id ? 'border-rojo-500' : 'border-rojo-900/20'}`} alt="" />
        ...
        <main className="flex-1 flex flex-col relative">
          ...
          <div className={`flex-1 overflow-y-auto p-10 space-y-8 no-scrollbar ${isDark ? 'bg-black' : 'bg-slate-50/30'}`}>
            {chatMessages.map(m => (
              <div key={m.id} className={`flex ${m.sender_id === currentUser?.id ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[75%] rounded-[1.5rem] px-6 py-4 text-sm shadow-xl ${
          ...
          <div className={`p-8 border-t z-10 ${isDark ? 'border-rojo-900/20 bg-black/60' : 'border-slate-100 bg-white'}`}>
            <div className="flex items-center space-x-4 bg-rojo-950/30 border border-rojo-900/20 rounded-[1.5rem] p-2 pr-4">
              <input 
                value={msg}
              ...
              />
              <button onClick={handleSend} className="bg-rojo-600 text-white p-3.5 rounded-2xl hover:bg-rojo-500 shadow-lg shadow-rojo-900/40 transition-all">
