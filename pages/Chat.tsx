...
const ChatPage: React.FC = () => {
  ...
  // Message Grouping Logic (Discord-style)
  ...
  const isNewBlock = (msg: ChatMessage, prev: ChatMessage | null) => {
    ...
    return diff > 4 * 60 * 1000; // 4 minutes
  };
        ...
        <main className="flex-1 flex flex-col relative bg-transparent overflow-hidden">
          {selectedUser ? (
                    ...
                    {group.messages.map((m, i) => {
                      ...
                      return (
                        <div 
                          ...
                          className={`group relative flex items-start px-4 py-0.5 hover:bg-zinc-900/30 transition-colors ${isNew ? 'mt-4' : ''}`}
                        >
                          {isNew ? (
                ...
                {pendingFiles.length > 0 && (
                  <div className="flex gap-2 mb-4 animate-in slide-in-from-bottom-2">
                    {pendingFiles.map((f, i) => (
                      <div key={i} className="relative w-20 h-20 rounded-2xl border border-zinc-800 overflow-hidden group shadow-2xl">
            ...
            <div className="flex-1 flex flex-col items-center justify-center opacity-20 p-20 text-center">
              ...
              <p className="text-[9px] uppercase font-bold mt-4 tracking-widest text-zinc-500">Select a contact to start messaging</p>
            </div>
          )}
        </main>
      </div>
        ...
        >
          <div className="p-2 flex flex-wrap gap-1.5 border-b border-zinc-900 mb-2">
            {QUICK_EMOJIS.map(e => (
              <button 
                key={e} 
                onClick={(ev) => { ev.stopPropagation(); reactToChatMessage(contextMenu.msgId, e); closeMenu(); }}
                className="hover:bg-rojo-600/20 hover:scale-125 p-2 rounded-lg transition-all text-lg"
              >
                {e}
              </button>
            ))}
             ...
             <input 
               ...
               onKeyDown={e => {
                 if (e.key === 'Enter' && customEmoji.trim()) {
                   reactToChatMessage(contextMenu.msgId, customEmoji.trim());
                   closeMenu();
                 }
               }}
               placeholder="Add custom reaction..."
               className="w-full bg-black border border-zinc-800 rounded-xl px-3 py-1.5 text-[10px] text-white outline-none focus:border-rojo-600 transition-colors"
             />
          </div>
      ...
      {profilePreviewId && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-6 animate-in fade-in duration-300 backdrop-blur-sm" onClick={() => setProfilePreviewId(null)}>
           <div 
             ...
             className={`w-full max-w-sm rounded-[2.5rem] overflow-hidden shadow-[0_0_100px_rgba(0,0,0,0.5)] border transition-all animate-in zoom-in duration-200 ${isDark ? 'bg-zinc-950 border-zinc-800' : 'bg-white border-zinc-100'}`}
           >
              {(() => {
                ...
                return (
                  <div className="flex flex-col">
                    ...
                    <div className="px-8 pb-8 -mt-10 relative">
                       <img src={pUser.avatarUrl} className="w-20 h-20 rounded-[1.5rem] border-4 border-zinc-950 shadow-2xl mb-4" alt="" />
                       ...
                       <div className="space-y-4 mb-8">
                          <div>
                            <p className="text-[8px] font-black uppercase text-zinc-600 tracking-widest mb-1">About Me</p>
                            <p className="text-xs text-zinc-400 leading-relaxed truncate-3-lines">{pUser.about || "This user is quite mysterious..."}</p>
                          </div>
                          <div>
                            <p className="text-[8px] font-black uppercase text-zinc-600 tracking-widest mb-1">Status</p>
                            <div className="flex items-center gap-2">
                               <div className={`w-2 h-2 rounded-full ${pUser.status === 'Active' ? 'bg-emerald-500' : 'bg-zinc-700'}`}></div>
                               <span className="text-[10px] font-bold text-zinc-500">{pUser.status === 'Active' ? 'Online' : 'Offline'}</span>
                            </div>
                          </div>
                       </div>

                       <div className="flex gap-2">
                          <button onClick={() => navigate(`/profile/${pUser.id}`)} className="flex-1 bg-rojo-600 text-white py-3 rounded-2xl text-[9px] font-black uppercase tracking-widest hover:bg-rojo-500 transition-all shadow-lg shadow-rojo-950/20">Full Profile</button>
...
const MessageContent: React.FC<{ 
  m: ChatMessage, isEditing: boolean, editVal: string, setEditVal: (v: string) => void, commitEdit: () => void, formatText: (t: string) => string, isMe: boolean 
}> = ({ m, isEditing, editVal, setEditVal, commitEdit, formatText, isMe }) => {
  return (
      ...
      {isEditing ? (
        <div className="space-y-2 mt-2">
          <textarea 
            ...
            className="w-full bg-black/40 border-2 border-rojo-600 rounded-2xl text-white outline-none resize-none p-4 text-sm font-medium" 
            autoFocus
            rows={3}
            onKeyDown={e => {
              ...
              if (e.key === 'Escape') setEditVal('');
            }}
          />
          <p className="text-[8px] text-zinc-600 font-bold uppercase">ESC to cancel • ENTER to save</p>
        </div>
      ) : (
        <div className="flex flex-col gap-2">
          <div 
            dangerouslySetInnerHTML={{ __html: formatText(m.content) }} 
            className={`text-sm leading-relaxed whitespace-pre-wrap break-words ${isMe ? 'text-zinc-200' : 'text-zinc-400'}`} 
          />
          {m.attachments?.map((url, i) => (
            <div key={i} className="mt-2 rounded-2xl overflow-hidden border border-zinc-800 shadow-2xl max-w-sm group">
    ...
    <div className="flex flex-wrap gap-1.5 mt-2">
      {Object.entries(m.reactions).map(([emoji, uids]) => (
        <button 
          ...
          className={`px-2 py-1 rounded-xl border-2 flex items-center gap-2 transition-all hover:scale-110 active:scale-90 ${
            uids.includes(currentUser?.id || '') 
              ? 'bg-rojo-600/10 border-rojo-600 text-rojo-500' 
...
const MenuBtn: React.FC<{ children: React.ReactNode, onClick: () => void, color?: string }> = ({ children, onClick, color }) => (
  <button 
    onClick={onClick}
    className={`w-full text-left px-3 py-2.5 text-[9px] font-black uppercase tracking-widest hover:bg-rojo-600 hover:text-white rounded-xl transition-all flex items-center ${color || 'text-zinc-400'}`}
  >
    {children}
  </button>
);
