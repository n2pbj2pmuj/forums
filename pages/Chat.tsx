...
const ChatPage: React.FC = () => {
  const { theme, users, chatMessages, fetchChatHistory, sendChatMessage, deleteChatMessage, editChatMessage, reactToChatMessage, currentUser, allChatPartners, friends, friendRequests, acceptFriendRequest, declineFriendRequest, blocks, unblockUser, clearNotification, notifications } = useAppState();
  ...
  const myBlocked = useMemo(() => {
    return blocks.map(b => users.find(u => u.id === b.blocked_id)).filter(Boolean) as User[];
  }, [blocks, users]);
  ...
  const isSelectedUserBlocked = useMemo(() => blocks.some(b => b.blocked_id === selectedUserId), [blocks, selectedUserId]);
  ...
  return (
    <Layout>
          ...
          <div className="flex-1 overflow-y-auto no-scrollbar">
              ...
              filteredContacts.map(user => {
                ...
                return (
                  <div 
                    ...
                    className={`flex items-center space-x-3 p-4 cursor-pointer transition-all border-b border-zinc-900/10 ${selectedUserId === user.id ? (isDark ? 'bg-rojo-600/10 border-r-4 border-rojo-600' : 'bg-rojo-50 border-r-4 border-rojo-600') : 'hover:bg-zinc-900/40'}`}
                  >
                    <div className="relative">
                      <img src={user.avatarUrl} className={`w-10 h-10 rounded-2xl border-2 ${selectedUserId === user.id ? 'border-rojo-500' : 'border-zinc-800'}`} alt="" />
                    ...
                    <div className="flex-1 min-w-0">
                      <p className={`text-xs font-black truncate flex items-center gap-2 ${selectedUserId === user.id ? 'text-rojo-500' : 'text-zinc-300'}`}>
                        {user.displayName}
                        {hasUnread && <span className="w-2 h-2 rounded-full bg-rojo-600 animate-pulse"></span>}
                      </p>
                      <p className="text-[9px] text-zinc-600 font-bold uppercase tracking-widest truncate">@{user.username}</p>
                    </div>
                  </div>
                );
              })
            ...
            {sidebarTab === 'friends' && (
              <div className="p-3 space-y-4">
                   ...
                   return (
                     <div key={req.id} className="p-3 bg-zinc-900/50 rounded-2xl border border-zinc-800 flex items-center justify-between">
                       ...
                       <div className="flex gap-1">
                          <button onClick={() => acceptFriendRequest(req.id)} className="p-1.5 bg-emerald-600 rounded-lg text-white">Accept</button>
                          <button onClick={() => declineFriendRequest(req.id)} className="p-1.5 bg-zinc-800 rounded-lg text-rojo-500">Ignore</button>
                       </div>
                     </div>
                   );
                 })}
                 ...
                 <div className="space-y-1">
                    {myFriends.map(user => (
                      <div key={user.id} onClick={() => { setSelectedUserId(user.id); setSidebarTab('chats'); }} className="flex items-center space-x-3 p-3 cursor-pointer rounded-2xl transition-all hover:bg-zinc-900/40">
                    ...
                    {myFriends.length === 0 && <p className="text-center py-10 opacity-20 text-[9px] font-black uppercase tracking-widest">No Friends Found</p>}
                 </div>
              </div>
            )}
            ...
            {sidebarTab === 'blocked' && (
              <div className="p-3 space-y-2">
                {myBlocked.map(user => (
                  <div key={user.id} className="flex items-center justify-between p-3 rounded-2xl bg-zinc-900/50 border border-zinc-800">
                     <div className="flex items-center gap-2">
                        <img src={user.avatarUrl} className="w-8 h-8 rounded-xl opacity-50 grayscale" alt="" />
                        <span className="text-[9px] font-black text-zinc-500 uppercase tracking-tighter">@{user.username}</span>
                     </div>
                     <button onClick={() => unblockUser(user.id)} className="text-[8px] font-black uppercase text-emerald-500 hover:underline">Unblock</button>
                  </div>
                ))}
                {myBlocked.length === 0 && <p className="text-center py-10 opacity-20 text-[9px] font-black uppercase tracking-widest">No Blocked Users</p>}
              </div>
            )}
          </div>
        </aside>
        ...
        <main className="flex-1 flex flex-col relative bg-transparent overflow-hidden">
          {selectedUser ? (
            <>
              <header className={`p-4 border-b flex items-center justify-between z-10 ${isDark ? 'border-zinc-800 bg-zinc-950/80 backdrop-blur-md' : 'border-zinc-100 bg-white/80'}`}>
                <div className="flex items-center space-x-4">
                  <img onClick={() => setProfilePreviewId(selectedUser.id)} src={selectedUser.avatarUrl} className="w-8 h-8 rounded-xl border border-zinc-800 cursor-pointer hover:opacity-80 transition-opacity" alt="" />
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <p className="text-[9px] text-zinc-500 font-bold uppercase tracking-widest">@{selectedUser.username}</p>
                    </div>
                  </div>
                </div>
                {isSelectedUserBlocked && <span className="text-[9px] font-black uppercase text-rojo-600 border border-rojo-950 px-2 py-1 rounded">User Blocked</span>}
              </header>
                        ...
                        <div key={m.id} onContextMenu={(e) => handleRightClick(e, m.id)} className={`group relative flex items-start px-4 py-0.5 hover:bg-zinc-900/10 transition-colors ${isNew ? 'mt-4' : ''}`}>
                          {isNew ? (
                            <div className="flex gap-4 w-full">
                              <img onClick={() => setProfilePreviewId(sender?.id || null)} src={sender?.avatarUrl} className="w-10 h-10 rounded-2xl border-2 border-zinc-900 shadow-xl shrink-0 mt-1 cursor-pointer hover:ring-2 ring-rojo-600 transition-all" alt="" />
                              <div className="flex-1 min-w-0">
              ...
              <div className={`p-6 border-t ${isDark ? 'border-zinc-800 bg-zinc-950/50' : 'border-zinc-100 bg-white'}`}>
                {isSelectedUserBlocked ? (
                  <div className="bg-zinc-900 rounded-2xl p-4 text-center border border-zinc-800 opacity-60">
                    <p className="text-[10px] font-black uppercase tracking-widest text-rojo-500">You cannot send messages to blocked users.</p>
                  </div>
                ) : (
                  <>
                    {pendingFiles.length > 0 && (
                      <div className="flex gap-2 mb-4">
                        {pendingFiles.map((f, i) => (
                          <div key={i} className="relative w-24 h-24 rounded-2xl border border-zinc-800 overflow-hidden group bg-zinc-900">
                             <img src={f} className="w-full h-full object-cover" alt="" />
                             <button onClick={() => setPendingFiles(p => p.filter((_, idx) => idx !== i))} className="absolute inset-0 bg-rojo-600/80 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"><svg className="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M6 18L18 6M6 6l12 12" /></svg></button>
                          </div>
                        ))}
                      </div>
                    )}
                    <div className="flex items-center gap-3 bg-zinc-900 border border-zinc-800 rounded-2xl px-4 py-2 focus-within:ring-2 ring-rojo-600/50 transition-all">
                      <button onClick={() => fileInputRef.current?.click()} className="text-zinc-500 hover:text-rojo-500 p-2"><svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" /></svg></button>
                      <input type="file" ref={fileInputRef} className="hidden" onChange={handleFileSelect} accept="image/*,video/*,audio/*" />
                      <input value={msg} onChange={e => setMsg(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSend()} placeholder={`Message @${selectedUser.displayName}...`} className="flex-1 bg-transparent border-none outline-none text-sm p-2 text-zinc-100 placeholder:text-zinc-700" />
                      <button onClick={handleSend} className="bg-rojo-600 text-white p-2.5 rounded-xl hover:bg-rojo-500 transition-all shadow-lg" disabled={!msg.trim() && pendingFiles.length === 0}><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M14 5l7 7m0 0l-7 7m7-7H3" /></svg></button>
                    </div>
                  </>
                )}
              </div>
            ...
            <div className="flex-1 flex flex-col items-center justify-center opacity-20 p-20 text-center">
              <p className="font-black uppercase tracking-[0.5em] text-xs">Select a Conversation</p>
            </div>
          )}
        </main>
      </div>
      ...
      {/* Context Menu */}
      {contextMenu && (
        <div style={{ top: contextMenu.y, left: contextMenu.x }} className="fixed z-[999] bg-zinc-950 border border-zinc-800 rounded-2xl shadow-2xl overflow-hidden w-64 p-2 animate-in zoom-in duration-100 origin-top-left">
          <div className="p-2 flex flex-wrap gap-1.5 border-b border-zinc-900 mb-2">
            {QUICK_EMOJIS.map(e => (
              <button key={e} onClick={() => { reactToChatMessage(contextMenu.msgId, e); closeMenu(); }} className="hover:bg-rojo-600/20 hover:scale-125 p-2 rounded-lg transition-all text-lg">{e}</button>
            ))}
            <button onClick={() => setShowFullEmojiPicker(!showFullEmojiPicker)} className={`p-2 rounded-lg transition-all text-zinc-500 hover:text-white ${showFullEmojiPicker ? 'bg-rojo-600 text-white' : 'hover:bg-zinc-900'}`}><svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" /></svg></button>
          </div>
          {showFullEmojiPicker && (
            <div className="px-2 mb-2 max-h-48 overflow-y-auto no-scrollbar border-b border-zinc-900 pb-2">
               <input autoFocus value={emojiSearch} onChange={e => setEmojiSearch(e.target.value)} onClick={e => e.stopPropagation()} placeholder="Search..." className="w-full bg-black border border-zinc-800 rounded-xl px-3 py-1.5 text-[10px] text-white outline-none mb-2" />
               <div className="grid grid-cols-6 gap-1">
                 {filteredEmojis.map(e => <button key={e} onClick={() => { reactToChatMessage(contextMenu.msgId, e); closeMenu(); }} className="hover:bg-zinc-800 p-1 rounded text-lg">{e}</button>)}
               </div>
            </div>
          )}
      ...
      {/* Profile Preview */}
      {profilePreviewId && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-6 backdrop-blur-sm animate-in fade-in" onClick={() => setProfilePreviewId(null)}>
           <div onClick={e => e.stopPropagation()} className={`w-full max-w-sm rounded-[2.5rem] overflow-hidden shadow-2xl border ${isDark ? 'bg-zinc-950 border-zinc-800' : 'bg-white border-zinc-100'}`}>
              {(() => {
                const pUser = users.find(u => u.id === profilePreviewId);
                if (!pUser) return null;
                return (
                  <div className="flex flex-col">
                    <div className="h-24 bg-rojo-950/20 relative">{pUser.bannerUrl && <img src={pUser.bannerUrl} className="w-full h-full object-cover opacity-60" alt="" />}</div>
                    <div className="px-8 pb-8 -mt-10 relative">
                       ...
                       <p className="text-[10px] text-zinc-500 font-black uppercase tracking-widest mb-6">@{pUser.username}</p>
                       <div className="flex gap-2">
                          <button onClick={() => navigate(`/profile/${pUser.id}`)} className="flex-1 bg-rojo-600 text-white py-3 rounded-2xl text-[9px] font-black uppercase tracking-widest hover:bg-rojo-500 transition-all">Full Profile</button>
      ...
      {isEditing ? (
        ...
        <div className="flex flex-col gap-2">
          ...
          {m.attachments?.map((url, i) => (
            <div key={i} className="mt-2 rounded-2xl overflow-hidden border border-zinc-800 shadow-2xl max-w-sm bg-zinc-900">
               <img src={url} className="w-full h-auto" alt="" />
            </div>
          ))}
  ...
  return (
    <div className="flex flex-wrap gap-1.5 mt-2">
      {Object.entries(m.reactions).map(([emoji, uids]) => (
        <button key={emoji} onClick={() => onReact(m.id, emoji)} className={`px-2 py-1 rounded-xl border-2 flex items-center gap-2 transition-all hover:scale-110 ${uids.includes(currentUser?.id || '') ? 'bg-rojo-600/10 border-rojo-600 text-rojo-500' : 'bg-zinc-900/50 border-zinc-800 text-zinc-500 hover:border-zinc-700'}`}>
