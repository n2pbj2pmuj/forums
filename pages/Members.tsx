            ...
            >
              ...
              <div className="flex flex-col items-center text-center space-y-5">
                <div className="relative">
                  <img src={user.avatarUrl} className={`w-24 h-24 rounded-3xl border-4 transition-all group-hover:scale-105 shadow-xl ${isDark ? 'border-rojo-950' : 'border-white'}`} alt="" />
                  <div className={`absolute -bottom-1 -right-1 w-5 h-5 rounded-full border-4 ${isDark ? 'border-black bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.5)]' : 'border-white bg-emerald-500'}`}></div>
                </div>
                
                <div className="min-w-0 w-full">
                  <h3 className={`text-xl font-black tracking-tight truncate transition-colors ${isDark ? 'text-white group-hover:text-rojo-500' : 'text-slate-900'}`}>{user.displayName}</h3>
