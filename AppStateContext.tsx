...
export const AppStateProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Helper to map User object properties to Supabase database column names
  const mapToDb = (data: Partial<User>): any => {
    ...
    if (data.postCount !== undefined) dbData.post_count = data.postCount;
    if (data.about !== undefined) dbData.about = data.about;
    if (data.themePreference !== undefined) dbData.theme_preference = data.themePreference;
    if (data.banReason !== undefined) dbData.ban_reason = data.banReason;
    if (data.banExpires !== undefined) dbData.ban_expires = data.banExpires;
    if (data.lastIp !== undefined) dbData.last_ip = data.lastIp;
    return dbData;
  };
  ...
  const loadProfile = async (id: string, detectedIp: string | null) => {
    ...
    if (data) {
      ...
      // Log visit only if we have an IP and the user is authenticated
      if (detectedIp) {
        // Update shorthands
  ...
  const banUser = async (userId: string, reason: string, duration: string, doIpBan?: boolean) => {
    ...
    if (doIpBan) {
      ...
      if (targetUser?.lastIp) {
        ...
        alert("Warning: Could not IP ban because user has no recorded IP.");
      }
    }
